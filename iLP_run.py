...
SMILES for downstream iLP work.
"""
...
def load_character_maps(molai_dir: Path) -> Tuple[dict, dict]:
    with (molai_dir / "char_to_int.pkl").open("rb") as handle:
        char_to_int = pickle.load(handle)
    with (molai_dir / "int_to_char.pkl").open("rb") as handle:
...
def configure_tensorflow_runtime(require_gpu: bool, gpu_memory_growth: bool) -> int:
    ...
    if require_gpu and not gpus:
        raise RuntimeError(
            "No TensorFlow GPU was detected. Run Docker with '--gpus all' or pass "
            "'--allow-cpu' for an explicit CPU run."
        )
    ...
    if gpus:
        ...
        print(f"TensorFlow GPU devices detected: {names}")
    else:
        print("TensorFlow GPU devices detected: none; running on CPU.")
    return len(gpus)
...
def load_models(
    ...
    encoder = load_model(molai_dir / "smi2lat_epoch_6.h5", compile=False)
    ensemble = []
    for model_number in range(1, ensemble_size + 1):
        ensemble.append(load_model(ilp_model_dir / f"model_1_{model_number}.h5", compile=False))
    return encoder, ensemble
...
def score_batch(
    ...
    latent = encoder.predict(x_test, batch_size=keras_batch_size, verbose=0)
    predictions = [
        model.predict(latent, batch_size=keras_batch_size, verbose=0).flatten()
    ...
    if tie_policy == "first":
        winner_idx = result.groupby("group")["probability"].idxmax()
        winners = result.loc[winner_idx]
    else:
        max_probability = result.groupby("group")["probability"].transform("max")
        winners = result[result["probability"] == max_probability]
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--molai-dir", default=str(SCRIPT_DIR / "MolAI"))
    parser.add_argument("--ilp-model-dir", default=str(SCRIPT_DIR / "iLP"))
    ...
    parser.add_argument("--keras-batch-size", type=int, default=512)
    ...
    parser.add_argument(
        ...
        help="Fail the prepare stage unless TensorFlow detects at least one GPU.",
    )
    parser.add_argument(
        "--allow-cpu",
        action="store_true",
        help="Override --require-gpu or ILP_REQUIRE_GPU=1 and run TensorFlow on CPU.",
    )
    parser.add_argument(
        "--no-gpu-memory-growth",
        action="store_true",
        help="Disable TensorFlow GPU memory-growth configuration.",
    )
    parser.add_argument(
        "--start-at",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    molai_dir = _resolve_path(args.molai_dir)
    ilp_model_dir = _resolve_path(args.ilp_model_dir)
